            ...
            <table class="add_tab">
                <tbody>
                    ...
                    <tr>
                        <th>类型：</th>
                        <td>
                            <input type="radio" name="field_type" value="input" onclick="showTr()"@if($field->field_type=='input') checked="checked"@endif>input　
                            <input type="radio" name="field_type" value="textarea" onclick="showTr()"@if($field->field_type=='textarea') checked="checked"@endif>textarea　
                            <input type="radio" name="field_type" value="radio" onclick="showTr()"@if($field->field_type=='radio') checked="checked"@endif>radio　
                        </td>
                    </tr>
                    <tr class="field_value" hidden="hidden">
                        <th>类型值：</th>
                        <td>
                            <input type="text" class="lg" name="field_value" value="{{$field->field_value}}">
                            <p><i class="fa fa-exclamation-circle yellow">只有在radio情况下才需要配置,格式 1|开启,0|关闭</i></p>
                        </td>
                    </tr>
                    <tr>
                        <th>排序：</th>
