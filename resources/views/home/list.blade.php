...
@section('info')
    <title>{{$field->cate_name}} - {{\Illuminate\Support\Facades\Config::get('web.seo_title')}}</title>
...
<div class="newblog left">
    @foreach($data as $k=>$v)
        <h2>{{$v->art_title}}</h2>
        <p class="dateview"><span>发布时间：{{date('Y-m-d H:i:s',$v->art_time)}}</span><span>作者：{{$v->art_editor}}</span><span>分类：[<a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a>]</span></p>
